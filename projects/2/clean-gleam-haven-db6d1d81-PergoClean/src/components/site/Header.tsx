      ...
      <div className="container mx-auto px-4">
        <div
          className={`flex items-center justify-between rounded-2xl px-4 py-2 transition-all duration-500 ${
            scrolled ? "glass-light shadow-premium" : ""
          }`}
        >
          <a href="#top" className="flex items-center gap-3">
            <img src={logo} alt="PergoClean logo" className="h-12 w-12 object-contain drop-shadow-[0_4px_18px_rgba(40,90,200,0.45)]" />
            <div className="leading-tight">
              <div className="font-display text-lg font-bold text-foreground">PergoClean</div>
          ...
          <nav className="hidden items-center gap-7 lg:flex">
            {links.map((l) => (
              <a
                key={l.href}
                href={l.href}
                className="text-sm font-medium text-foreground/80 transition hover:text-[color:var(--aqua)]"
              >
                {l.label}
              </a>
            ))}
          ...
          <a
            href="#fiyat"
            className="hidden rounded-full bg-aqua-grad px-5 py-2.5 text-sm font-semibold text-white shadow-glow transition hover:scale-105 lg:inline-flex"
          >
            Anında Fiyat Al
          </a>
